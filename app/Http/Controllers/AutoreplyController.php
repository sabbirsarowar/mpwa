...
class AutoreplyController extends Controller
{
    ...
    {
        $autoreply = $request->user()->autoreplies()->find($request->id);
        ...
        if ($autoreply) {
            ...
            return response()->json([
                'error' => false,
                'message' => __('Your auto reply was deleted')
            ]);
        }

        return response()->json([
            'error' => true,
            'message' => __('Auto reply not found or you do not have permission to delete it.')
        ], 404);
    }
